...
class _CroppingImageViewState extends State<CroppingImageView> {
    ...
    if (selectedImageRatio == ImageRatio.FREE) {
      ...
      if (cropSizeWidth < minCropSizeWidth ||
              ...
              leftTopDX + cropSizeWidth + widget.squareCircleSize! >
                  stackGlobalKey.globalPaintBounds!.width // this condition checks the right top crop button is outside the screen.
          ) {
        cropSizeWidth = _previousCropWidth;
        cropSizeHeight = _previousCropHeight;
        leftTopDX = _previousLeftTopDX;
      ...
      SetImageRatio.setRightBottomCropButtonPosition(rightBottomDx: leftTopDX + cropSizeWidth, rightBottomDy: leftTopDY + cropSizeHeight);
    }
  }
  ...
  void _manageLeftBottomButtonDrag(state, DragUpdateDetails details, DragDirection dragDirection) {
    ...
    if (selectedImageRatio == ImageRatio.FREE) {
      ...
      SetImageRatio.setRightBottomCropButtonPosition(rightBottomDx: leftBottomDX + cropSizeWidth, rightBottomDy: leftBottomDY);
    }
  }
  ...
  void _manageRightTopButtonDrag(state, DragUpdateDetails details, DragDirection dragDirection) {
    ...
    if (selectedImageRatio == ImageRatio.FREE) {
      ...
      if (cropSizeWidth < minCropSizeWidth ||
              cropSizeHeight < minCropSizeHeight ||
              (rightTopDX - cropSizeWidth) < 1 // this condition checks the left top crop button is outside the screen.
          ) {
        cropSizeWidth = _previousCropWidth;
        cropSizeHeight = _previousCropHeight;
        rightTopDX = _previousRightTopDX;
      ...
      SetImageRatio.setLeftBottomCropButtonPosition(leftBottomDx: leftTopDX, leftBottomDy: leftTopDY + cropSizeHeight);
      SetImageRatio.setRightBottomCropButtonPosition(rightBottomDx: rightTopDX, rightBottomDy: rightTopDY + cropSizeHeight);
    }
  }
  ...
  void _manageRightBottomButtonDrag(state, DragUpdateDetails details, DragDirection dragDirection) {
    ...
    if (selectedImageRatio == ImageRatio.FREE) {
      ...
      SetImageRatio.setRightTopCropButtonPosition(rightTopDx: rightBottomDX, rightTopDy: rightBottomDY - cropSizeHeight);
      SetImageRatio.setLeftTopCropButtonPosition(leftTopDx: rightTopDX - cropSizeWidth, leftTopDy: rightTopDY);
      SetImageRatio.setLeftBottomCropButtonPosition(leftBottomDx: leftTopDX, leftBottomDy: leftTopDY + cropSizeHeight);
    }
  }
  ...
  void _manageSquareDrag(state, DragUpdateDetails details, DragDirection dragDirection) {
    ...
    if (globalPositionDX < -9 || (globalPositionDX + cropSizeWidth + (widget.squareCircleSize! / 5)) > MediaQuery.of(context).size.width) {
      globalPositionDX = leftTopDX;
    }

    if (globalPositionDY < -9 || (globalPositionDY + cropSizeHeight + (widget.squareCircleSize! / 5)) > imageViewMaxHeight) {
